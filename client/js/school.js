...
/*-- -- -- On créé une fonction qui permet d'envoyer un nouveau film au server -- -- --*/
function addSchool(){    
...
function deleteScholl(id){
    xhttp.open('DELETE','/school/'+id,true);
    xhttp.send();
    window.location.href='/pages/school.html';
};
...
function addOneLine(data){
    var tab=document.querySelector('#School');
    ...
    link.href = '/pages/detailSchool.html' + data._id;
...
|                                                                       |
|   Je crée l'écouteur d'évenement associé au clic du bouton validation |
